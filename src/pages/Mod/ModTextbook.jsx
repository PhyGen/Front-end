...
const ModTextbook = () => {
  ...
  const handleEdit = (id) => {
    ...
    if (textbook) {
      setEditId(id);
      setEditRow({ id: id, name: textbook.name });
    }
  };
                  ...
                  editId === t.id ? (
                    <tr key={t.id} className="bg-yellow-50">
                      ...
                      <td className="px-4 py-2 border">
                        <Input value={editRow.name} onChange={e => setEditRow(r => ({ ...r, name: e.target.value }))} placeholder="Nhập tên sách giáo khoa" />
                      </td>
                      <td className="px-4 py-2 border">{formatDate(t.createdAt)}</td>
                      <td className="px-4 py-2 border">{formatDate(t.updatedAt)}</td>
                      <td className="px-4 py-2 border flex gap-2">
                        <Button size="sm" className="bg-blue-500 hover:bg-blue-600 text-white" onClick={handleEditSave} disabled={!editRow.name.trim()}>Save</Button>
                    ...
                    <tr key={t.id} className="even:bg-slate-50">
                      ...
                      <td className="px-4 py-2 border">{formatDate(t.updatedAt)}</td>
                      <td className="px-4 py-2 border flex gap-2">
                        <Button size="sm" className="bg-blue-500 hover:bg-blue-600 text-white" onClick={() => handleEdit(t.id)}>Edit</Button>
                        <Button size="sm" className="bg-red-500 hover:bg-red-600 text-white" onClick={() => handleDelete(t.id)}>Delete</Button>
                      </td>
                    </tr>
                  )
                ))}
